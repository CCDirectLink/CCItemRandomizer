export async function extractMarkers(spoilerLog, mapNames, mapData, areaNames, areas) {
    ...
    for (const check of spoilerLog) {
        ...
        markers[map.attributes.area].push({key: check.type === 'chest' ? "CHEST" : "UNKNOWN", x: mx, y: my, level: floor.level, index: markers[map.attributes.area].length, map: check.map, mapId: check.mapId}); 
    }
    return markers;
}